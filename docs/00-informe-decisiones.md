...
| D17 | **Eliminación de visualizaciones duplicadas entre módulos + interactividad + completitud** | Se deduplica: Dashboard deja KPIs de las 4.1/4.2; Tendencias deja SOLO heatmaps (tipo×mes, ámbito×mes, área×canal); Ámbito deja ranking+ámbito×tipo+heatmap ámbito×mes+mediana×ámbito; Áreas deja ranking+área×tipo+heatmap área×ámbito+mediana×área (dueño único de la matriz de tiempo). Se implementa click-to-filter (`ChartContainer.clicFiltro` → `FilterService.alternarCategoria`), KPIs calculados client-side desde `/api/registros` (filtrables), nuevo módulo "Red y Cobertura" (`/red`: aseguradora/regional/tipo_usuario/vencimiento + aseguradora×tipo), análisis textual completo (explorador de top palabras por dimensión + buscador por keywords con listado de casos), y se unifica el filtro de tipo en `tipo_pqrs_grupo` (antes granular `tipo_pqrs`) para que las selecciones del filtro-bar, clicks y cruces coincidan. Backend: EDA ampliado a 16 cruces + `tiempo_respuesta.por_ambito/por_area` + `textual.json` + `registros.json`. |
| D18 | **Fix de deep links del SPA en Vercel (404 en `/dashboard`, `/red`, etc.)** | `app.frontend(path, fallback="index.html")` no funciona como catch-all en FastAPI 0.141: responde 404 para subrutas que no existen como archivo (fallaba también localmente). Se reemplaza por un catch-all `@app.get("/{ruta_restante:path}")` registrado como última ruta: sirve archivos reales de `public/` con FileResponse y devuelve `index.html` como fallback; `/api/*` sin coincidencia sigue devolviendo 404. Se corrige el orden frente a `/health` (registrado antes del catch-all) garantizando `{"status":"ok"}`. |
